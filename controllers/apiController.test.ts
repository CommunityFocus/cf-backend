	...
	describe("slugHandler", () => {
		...
		test("should return a slug", async () => {
			...
			expect(res.json).toHaveBeenCalledWith({
				slug: "big-blue-butterfly",
			});
		});

		test("should regenerate slug until it finds one that isn't already taken", async () => {
			req.timerStore = {
				"first-room-name": {
					users: [],
					timer: undefined,
		...
		test("should return an error and status 429 if it cannot generate a slug that isn't already taken after 50 tries", async () => {
			/*
		...
		test("should return an error and status 500 if timer store is missing on request object", async () => {
